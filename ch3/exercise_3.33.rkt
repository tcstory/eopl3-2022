...
(define (extend-env-rec p-name b-vars p-body saved-env)
               (lambda (search-var orig-env)
                 (if (eqv? search-var p-name)
                     (proc-val (procedure b-vars p-body orig-env))
                     (my-apply-env saved-env search-var orig-env))))

(define (extend-env-rec* p-names b-varss p-bodys saved-env)
  (if (null? p-names)
      saved-env
      (let ((new-env (extend-env-rec (car p-names) (car b-varss) (car p-bodys) saved-env)))
        (extend-env-rec* (cdr p-names) (cdr b-varss) (cdr p-bodys) new-env))))
